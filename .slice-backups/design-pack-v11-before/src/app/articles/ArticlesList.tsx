...
export function ArticlesList({ articles, initialCategory }: { articles: Article[]; initialCategory?: string }) {
  ...
  return <section className={styles.writing} aria-labelledby="all-writing">
    ...
    <div className={styles.list}>{filtered.map((article, index) => <article className={styles.row} key={article.slug}>
      <Link className={styles.rowImage} href={`/articles/${article.slug}`} aria-label={`Read ${article.title}`}><Image src={article.image ?? images[index % images.length]} alt={article.imageAlt ?? ""} fill sizes="(max-width: 700px) 100vw, 250px" /></Link>
