...
export default async function EditArticlePage({ params, searchParams }: { params:Promise<{id:string}>; searchParams:Promise<{error?:string}> }) {
  ...
  if (!article) notFound();
  return <><div className={styles.heading}><div><h1>Edit article</h1><p>Changes become visible when the article is published.</p></div></div><ArticleForm article={article} authors={authors ?? []} categories={categories ?? []} error={query.error} /></>;
}
