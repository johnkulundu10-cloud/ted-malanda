...
export function ArticleForm({ article, authors, categories, error }: { article: RecordValue | null; authors: Choice[]; categories: Choice[]; error?: string }) {
  ...
  return <>
    ...
    <form className={styles.form} action={saveArticle} encType="multipart/form-data">
      ...
      <div className={styles.field}><label htmlFor="content">Article</label><textarea className={styles.content} id="content" name="content" defaultValue={value(article,"content")} placeholder={'Write normal paragraphs separated by an empty line.\n\nUse ## before a section title.'} /></div>
      <div className={styles.columns}><div className={styles.field}><label htmlFor="author_id">Author</label><select id="author_id" name="author_id" defaultValue={value(article,"author_id")}>{authors.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></div><div className={styles.field}><label htmlFor="category_id">Category</label><select id="category_id" name="category_id" defaultValue={value(article,"category_id")}>{categories.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select></div></div>
      <div className={styles.columns}><div className={styles.field}><label htmlFor="status">Status</label><select id="status" name="status" defaultValue={value(article,"status") || "draft"}><option value="draft">Draft</option><option value="published">Published</option></select></div><div className={styles.field}><label htmlFor="published_at">Publication date</label><input id="published_at" name="published_at" type="datetime-local" defaultValue={published} /></div></div>
      ...
      <div className={styles.checks}><label><input type="checkbox" name="is_archived" defaultChecked={Boolean(article?.is_archived)} /> Archive article</label><label><input type="checkbox" name="is_featured" defaultChecked={Boolean(article?.is_featured)} /> Feature this article</label></div>
