...
export default async function Home() {
  ...
  const archiveLead = articles.find((article) => article.archived) ?? articles[4] ?? articles[1] ?? articles[0];
  const leadStories = [{ ...articles[0], label: "Latest story", image: articles[0]?.image ?? fallbacks[0] }, { ...archiveLead, label: "From the archive", image: archiveLead?.image ?? fallbacks[1] }];
  const visualStories = articles.slice(1, 5).map((article, index) => ({ ...article, image: article.image ?? fallbacks[index % fallbacks.length] }));
