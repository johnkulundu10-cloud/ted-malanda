...
export default async function NewArticlePage({ searchParams }: { searchParams: Promise<{ error?:string }> }) {
  ...
  return <><div className={styles.heading}><div><h1>New article</h1><p>Images and original publication details are optional.</p></div></div><ArticleForm article={null} authors={authors ?? []} categories={categories ?? []} error={query.error} /></>;
}
