  ...
  return <section className={styles.archive} aria-label="Archived articles">
    ...
    <div className={styles.list}>{visible.map((article) => {
      ...
      return <article className={styles.row} key={article.slug}>
        <Link className={styles.image} href={`/articles/${article.slug}`} aria-label={`Read ${article.title}`}><Image src={article.image ?? images[originalIndex % images.length]} alt={article.imageAlt ?? ""} fill sizes="(max-width: 700px) 100vw, 260px" /></Link>
