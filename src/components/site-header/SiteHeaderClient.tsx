...
export function SiteHeaderClient({ categories }: { categories: PublishedCategory[] }) {
  const [open, setOpen] = useState(false);
  const navigation = [
    ...primaryNavigation,
    ...categories.map((category) => ({ href: `/articles?category=${encodeURIComponent(category.name)}#all-writing`, label: category.name, mobileOnly: true })),
  ...
  return <header className={styles.header}>
    <div className={styles.inner}>
      ...
      <nav id="mobile-navigation" className={`${styles.navigation} ${open ? styles.navigationOpen : ""}`} aria-label="Primary navigation">
        {navigation.map((item) => <Link className={("mobileOnly" in item && item.mobileOnly) ? styles.mobileOnly : undefined} onClick={() => setOpen(false)} key={item.href} href={item.href}>{item.label}</Link>)}
      </nav>
