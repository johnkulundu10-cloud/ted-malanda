...
import { ImageIcon, Search, Star, X } from "lucide-react";
import { useState } from "react";
import { bypassImageOptimizer } from "@/lib/images";
import styles from "@/app/admin/admin.module.css";

export type AdminArticleRow={id:string;title:string;slug:string;excerpt:string;status:string;published_at:string|null;category:string;image:string|null;imageAlt:string;featured:boolean};
...
export function AdminArticlesTable({articles}:{articles:AdminArticleRow[]}) {
  ...
  return <>
    ...
    <section className={`${styles.table} ${styles.articleTable}`}>{filtered.length?filtered.map((article)=><Link className={styles.adminArticleRow} href={`/admin/articles/${article.id}`} key={article.id}>
      ...
      <div className={styles.adminRowCopy}><strong>{article.title}</strong><small>{article.excerpt||"No description added yet."}</small><span>{article.category||"Uncategorised"}</span></div>
